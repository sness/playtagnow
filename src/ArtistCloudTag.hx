...
class ArtistCloudTag extends Sprite {
...
// 	var _tag : String;
	...
	public function new (x_:Float, y_:Float, artist_:Artist, total_num_:Int) {
		super();
		...
		_text_name.text = _artist.getPrettyName();
...
//   		trace("down=" + _artist.getName());
		if (e.shiftKey) {
			CurrentArtists.toggleArtist(_artist);
		} else {
			CurrentTags.unselectAll();
			CurrentArtists.selectArtist(_artist);
		}
	}

	public function getTag():String {
		return _artist.getPrettyName();
	}
...
// 		trace("gbe " + _tag + " py=" + rr(_position.y) + " tw2=" + rr(_text_name.height / 2.0) + " ret=" + rr(_position.y + (_text_name.height / 2.0)));
	...
	public function overlapsWith(other:ArtistCloudTag) {
