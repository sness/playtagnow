...
class SongCloudTag extends Sprite {
...
// 	var _tag : String;
	...
	// Is this selected by the current combination of CurrentSongs and CurrentTags?
	var _selected : Bool;

	public function new (x_:Float, y_:Float, song_:Song, total_num_:Int) {
		super();
		...
		_text_name.text = _song.getPrettyTitle();
...
//  		trace("down=" + _song.getTitle());

// 		SongCloud.unselectAll();

		// Play this song
 		CurrentSong.changeSong(_song.getFirstClip());
...
// 		// Deselect all Artists and Tags
// 		CurrentArtists.unselectAll();
// 		CurrentTags.unselectAll();

		// Find out which Artist this Song belongs to and select it
// 		var artist:Artist = ;
// 		CurrentArtists.addArtist(_song.getArtist());

		// Find out which Tags this song has and select them
...
// 		trace("gbe " + _tag + " py=" + rr(_position.y) + " tw2=" + rr(_text_name.height / 2.0) + " ret=" + rr(_position.y + (_text_name.height / 2.0)));
	...
	public function overlapsWith(other:SongCloudTag) {
...
// 		trace("bt=" + rr(b_top) + " bb=" + rr(b_bottom) + " bl=" + rr(b_left) + " br=" + rr(b_right));
	...
	public function randomOrder():Void {
		setOrigXY(Math.random() * ArtistCloud.getWidth(), Math.random() * ArtistCloud.getHeight());
// 		_orig_x = Math.random() * ArtistCloud.getWidth();
// 		_orig_y = Math.random() * ArtistCloud.getHeight();
	}
